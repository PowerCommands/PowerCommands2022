...
[PowerCommandDesign(description: "List the latest added knowledge documents.",
    flags: "!days|!weeks|url|path|onenote|file",
...
public class LatestCommand : DisplayCommandsBase
{
    ...
    {
        var sourceType = Input.GetFlagValue(new[] { "url", "path", "url", "file" });
        var dayFactor = Input.HasFlag("weeks") ? 7 : 1;
        var number = 0;
        if (int.TryParse(Input.GetFlagValue("weeks"), out var index)) number = index;
        if (int.TryParse(Input.GetFlagValue("days"), out var index2)) number = index2;
